function [logprior, der_logprior] = der_logprior_fn(x,options)

model_choice = options.model_choice;

% Dimension of theta
p = length(x);
der_logprior = zeros(1,p);
...
for j=1:6
    ...
    der_logprior(j) = 1/IG2./exp(x(j)) - IG1;
end
...
if model_choice>0
    ...
    for j=diags
        ...
        der_logprior(j) = 1 - exp(2*x(j))/(C0^2); %from truncated normal with log taken
    end
    for j=offdiag
        logprior(j) = log_normpdf(x(j),0,C0); %Normal with no logs
        der_logprior(j) = - x(j)/(C0^2); %from normal with no logs
    end
end
