function [theta, log_weights, log_evidence, count_loglike, error_flag] = stratified_SLICE_fixed(loglike_fn,logprior_fn,simprior_fn,options,verbose)
...
while t<=T
    ...
    if levels(t) ~= Inf
        ...
        parfor i=1:N
            for z = 1:R(t)
                [theta_curr(i,:), count_loglike_new] = slicesample(theta_curr(i,:),1,'logpdf',logtarget,'width',w(t)*std_pop{t});
                loglike_curr(i,:) = loglike_fn(theta_curr(i,:),options);
                logprior_curr(i,:) = logprior_fn(theta_curr(i,:),options);
